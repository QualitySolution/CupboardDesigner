						...
						if ((Nomenclature.NomType)ComponentsStore.GetValue(iter, (int)ComponentCol.nomenclature_type) == Nomenclature.NomType.cube) {
							...
							if(ComponentsStore.IterHasChild(iter)) {
								...
								do { //Adding every nomenclature for basis
									...
									if(HasValue) {
										if(!InDB)
											sql = "INSERT INTO order_basis_details (order_id, basis_id, nomenclature_id, " +
												"count, price, comment, discount, facing_id, material_id) " +
												"VALUES (@order_id, @basis_id, @nomenclature_id, @count, " +
												"@price, @comment, @discount, @facing_id, @material_id)";
										else
